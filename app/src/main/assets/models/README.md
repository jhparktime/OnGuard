...
| 파일명 | 용도 |
|--------|------|
| `model.gguf` | Qwen 2.5 (1.5B) GGUF 모델 - 온디바이스 스캠 탐지 |

**중요:** 앱은 **llama.cpp** (java-llama.cpp)로 GGUF 모델을 로드합니다.  
예: `qwen2.5-1.5b-instruct-q4_k_m.gguf` 등을 `model.gguf`로 이름 변경 후 이 폴더에 넣습니다.
...
3. 이 폴더 (`app/src/main/assets/models/`)에 복사
